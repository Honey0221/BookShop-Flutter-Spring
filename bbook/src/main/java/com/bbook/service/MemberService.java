...
public class MemberService {
    ...
    // 소셜 로그인 처리
    @Transactional
    public Map<String, Object> processSocialLogin(String email) {
			...
			try {
				Member member = memberRepository.findByEmail(email)
					.orElseGet(() -> {
							...
							// 소셜 로그인으로 가입한 신규 회원에게 쿠폰 발급
							couponService.createBasicCoupons(savedMember);
							
							return savedMember;
					});
				
				// 닉네임이 설정되지 않은 경우 신규 사용자로 간주
				isNewUser = member.getNickname() == null || member.getNickname().isEmpty();
				...
				System.out.println("소셜 로그인 처리 중 오류 발생" + e.getMessage());
